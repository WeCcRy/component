  ...
  function visitNode(node, parent, isReactivityTransform = false) {
    if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) && [DEFINE_PROP, DEFINE_PROP_DOLLAR].includes(node.expression.escapedText)) {
      if (edition === "kevinEdition") {
        ...
        const name = node.arguments[0] && ts.isStringLiteral(node.arguments[0]) ? node.arguments[0].text : ts.isVariableDeclaration(parent) && ts.isIdentifier(parent.name) ? getText(parent.name, { ts, sfc }) : void 0;
        const prop = ts.isVariableDeclaration(parent) && ts.isIdentifier(parent.name) ? getText(parent.name, { ts, sfc }) : void 0;
        ...
        if (optionArg) {
          for (const property of optionArg.properties) {
            if (ts.isPropertyAssignment(property) && ts.isIdentifier(property.name)) {
              if (getText(property.name, { ts, sfc }) === "required" && property.initializer.kind === ts.SyntaxKind.TrueKeyword)
                required = true;
              if (ts.isIdentifier(property.name) && getText(property.name, { ts, sfc }) === "default")
        ...
        const name = ts.isIdentifier(parent.name) ? getText(parent.name, { ts, sfc }) : void 0;
        defineProps.push({
          name,
          prop: name,
