...
function transform(nodes, ctxMap, options) {
  ...
  for (const { attribute: attribute2, node: node2 } of nodes) {
    const isNative = isNativeFormElement(getTagName(node2, options));
    ...
    if (name.startsWith(`${prefix}model:`) || isArrayExpression) {
      let isDynamic = false;
      const [attributeName2, ...modifiers] = name.slice(offset).split(/\s/)[0].replace(/^\$(.*)\$/, (_, $1) => {
        isDynamic = true;
      ...
      if (isArrayExpression) {
        ...
        if (attributeName2) {
          if (isJsxExpression(attribute2?.initializer) && attribute2.initializer.expression) {
            result.push(":", [
            ...
            modifiersResult.push(
              ...
              ...modifiers.flatMap((modify, index) => [
                ...
                [
                  modify,
                  source,
                  start + offset + attributeName2.length + 1 + (index ? modifiers.slice(0, index).join("").length + index : 0),
                  allCodeFeatures
                ],
                modify ? ": true, " : `'`
              ]),
              `} satisfies typeof ${ctxMap.get(node2)}.props.${attributeName2}Modifiers}`
            );
          }
        }
      }
      emitsResult.push(`'onUpdate:${attributeName2}': () => {}, `);
    } else {
      const [, ...modifiers] = name.split("_");
      const result2 = [];
      result2.push(
        ...isNative ? [[modelValue, source, start + 2, allCodeFeatures]] : [
        ...
        result2.unshift(
          ...
          ...modifiers.flatMap((modify, index) => [
            ...
            [
              modify,
              source,
              start + offset + (index ? modifiers.slice(0, index).join("").length + index : 0),
              allCodeFeatures
            ],
            modify ? ": true, " : `'`
          ]),
          `} satisfies `,
          isNative ? "{ trim?: true, number?: true, lazy?: true}" : `typeof ${ctxMap.get(node2)}.props.modelValueModifiers`,
          `} `
        );
      }
      if (!isNative) {
        result2.unshift(`{...{'onUpdate:${modelValue}': () => {} }} `);
      }
      replaceSourceRange(codes, source, start, attribute2.name.end, ...result2);
    }
  }
...
function transformCustomDirective(attributes, options) {
  ...
  attributes.forEach((attribute) => transform2(attribute, options));
  options.codes.push(`
...
function transform2(attribute, options) {
  ...
  replaceSourceRange3(
    ...
    `={[`,
    [`v`, source, start, { ...allCodeFeatures2, verification: false }],
    [
      directiveName,
      source,
      start + 2,
      { ...allCodeFeatures2, verification: false }
    ],
    `,`,
    ...
    ...arg !== void 0 ? [
      ...
      [arg, source, offset + 1, allCodeFeatures2],
      [
        `'`,
        source,
        offset + arg.length,
  ...
  for (const { node, attribute } of nodes) {
    if (attribute.initializer && isJsxExpression(attribute.initializer) && attribute.initializer.expression && (ts.isFunctionExpression(attribute.initializer.expression) || ts.isArrowFunction(attribute.initializer.expression))) {
      replaceSourceRange4(
        ...
        attribute.end,
        "{...({ ",
        ["ref", source, getStart(attribute.name, options), allCodeFeatures3],
        ": ",
        [
...
function transformVBind(nodes, options) {
  ...
  for (const { attribute } of nodes) {
    ...
    if (attributeName.includes("_")) {
      replaceSourceRange5(codes, source, start + attributeName.indexOf("_"), end);
    }
  }
}
...
function transformVFor(nodes, options, hasVForAttribute) {
  ...
  nodes.forEach(({ attribute, node, parent }) => {
    ...
    replaceSourceRange6(codes, source, node.pos, node.pos, ...result);
    replaceSourceRange6(
      codes,
      source,
      node.end - 1,
...
function transformVIf(nodes, options) {
  ...
  nodes.forEach(({ node, attribute, parent }, index) => {
    ...
    if ([`${prefix}if`, `${prefix}else-if`].includes(
      ...
      replaceSourceRange7(codes, source, node.end, node.end, parent ? "}" : "");
    }
    replaceSourceRange7(
      codes,
      source,
