...
type FactoryOutput<
	UserOptions,
	Return
> = [never] extends UserOptions ? () => Return : undefined extends UserOptions ? (options?: UserOptions) => Return : (options: UserOptions) => Return;
type Unplugin<UserOptions> = {
	instance: UnpluginInstance<UserOptions, boolean> | UnpluginCombineInstance<any>
	options?: UserOptions
};
type Awaitable<T> = T | Promise<T>;
type OptionsPlugin = Awaitable<Plugin> | Unplugin<any> | Awaitable<OptionsPlugin[]>;
...
interface UnpluginCombineInstance<UserOptions> {
	rollup: FactoryOutput<UserOptions, Promise<Plugin$1[]>>;
	rolldown: FactoryOutput<UserOptions, Promise<Plugin$2[]>>;
	webpack: FactoryOutput<UserOptions, WebpackPlugin>;
	rspack: FactoryOutput<UserOptions, RspackPlugin>;
	vite: FactoryOutput<UserOptions, Promise<Plugin$3[]>>;
	esbuild: FactoryOutput<UserOptions, Plugin$4>;
	raw: Factory<UserOptions>;
	plugins: FactoryOutput<UserOptions, OptionsPlugin>;
}

export type { Awaitable as A, CombineOptions as C, Factory as F, OptionsPlugin as O, PluginType as P, RemoveFalsy as R, UnpluginCombineInstance as U, WebpackPlugin as W, PluginMap as a, RspackPlugin as b, Plugin as c, FactoryOutput as d, Unplugin as e };
